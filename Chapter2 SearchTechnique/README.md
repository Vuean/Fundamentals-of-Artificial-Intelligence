...
有限深度优先搜索的基本思想：深度截止于k的深度有限搜索（k以下深度的节点不再扩展），防止深度优先搜索陷入无限深度。
...
3. 把OPEN表中的第1个节点取出放入CLOSED表中，并按顺序冠以编号n；
...
> 迭代加深的深度优先搜索 DFS-ID(DFS With Iterative Deepening)

迭代加深的深度优先搜索的基本思想：结合了广度优先和深度优先搜索两者的优点。
...
|空间复杂度|O(b^d)|O(b*m)|O(bd)orO(d)|

当路径代价是节点深度的非递减函数时，DFS-ID是最优的。

在最坏的情况下，所有的盲目搜索(DFS、BFS、DFS-ID)，都显示出了指数级的时间复杂度。
